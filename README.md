...
**Novel KB**（InaWrite）是专为小说作者、世界构建爱好者和跑团设定者打造的本地知识库管理工具。它帮助你在一个统一的界面中管理角色、章节、伏笔、世界观设定和资源能力，并通过关系图谱、看板视图、树形结构等方式可视化你的创作世界。
...
- [ ] **大纲编辑器**：树形大纲结构，拖拽调整章节顺序，支持多级标题
  - 大纲节点可绑定角色出场、伏笔埋设/回收、世界观引入
  - 大纲与章节双向同步，大纲变化自动更新章节序号
- [ ] **角色时间线弧光**：按章节可视化角色成长轨迹
  - 横向时间轴展示角色状态变化（存活→死亡、所在地点迁移）
  - 弧光阶段标记（对应角色 arc 字段）
  - 关键事件标注（获得能力、关系变化、伏笔触发）
- [ ] **写作会话统计**：记录每次写作的时间、字数、章节
  - 日/周/月维度统计图表
  - 写作习惯分析（最佳写作时段、平均速度）
  - 与写作日历热力图联动
- [ ] **章节草稿编辑器**：内置富文本/Markdown 编辑器直接写章节内容
  - 侧边栏显示当前章节关联的角色、伏笔、地点
  - 写作时自动提示：引用角色名时弹出角色卡片预览
  - 字数实时统计，达到目标时提醒
- [ ] **标签/分类系统**：为角色、章节、伏笔、设定添加自定义标签
  - 全局标签云视图
  - 按标签筛选任意模块
  - 标签关联分析（哪些标签常同时出现）
- [ ] **导出增强**：支持导出为 DOCX、PDF、EPUB 格式
  - 章节内容按序合并导出
  - 可选包含角色列表/世界观附录
  - 自定义排版模板
...
- [ ] **AI 章节续写**：基于已有章节内容、角色设定、进行中伏笔，AI 辅助续写
  - 保持角色语言风格（voice 字段）
  - 自动推进伏笔状态
  - 支持多轮对话式调整
- [ ] **AI 一致性检查**：扫描全文检测设定矛盾
  ...
  - 资源状态矛盾（已消耗物品再次使用）
  - 生成一致性报告，标注问题位置
- [ ] **AI 世界观补全**：根据已有设定自动推断和补全
  - 检测设定间的逻辑空白（如只有"国家"没有"城市"）
  - 建议关联关系（检测到文本中两个设定有互动但未建立关联）
- [ ] **实时协作编辑**：CRDT 多人同步
  - WebSocket 实时同步
  - 冲突自动合并
  - 操作历史可视化（谁在什么时候改了什么）
...
- [ ] **Electron 桌面应用打包**：Windows/Mac/Linux 独立应用
  - 系统托盘常驻
  - 原生文件系统访问（直接读写本地 Markdown 文件）
  - 自动保存和崩溃恢复
- [ ] **移动端适配**：PWA 离线可用
  - 响应式布局适配手机/平板
  - 离线数据同步
